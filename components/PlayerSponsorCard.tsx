...
// =============================================================================
// components/PlayerSponsorCard.tsx — Player-facing sponsor share card
// =============================================================================
...
//
// All work happens client-side. The flyer generator (lib/sponsorFlyerPdf.ts)
// already runs in-browser via jsPDF — same code path the coach uses.
// =============================================================================

import { useEffect, useState } from "react";
import QRCode from "qrcode";
import { generateSponsorFlyerPDF } from "@/lib/sponsorFlyerPdf";
...
export default function PlayerSponsorCard(props: Props) {
  const url = `https://app.earn2keep.com/sponsor/${props.token}`;
  ...
  const handleShare = async () => {
    ...
    try {
      await navigator.share({
        title: `Sponsor ${props.playerFirstName}'s earn²keep page`,
  ...
  return (
    <div className="player-sponsor-card">
      {/* QR — large, centered, on a white card so it scans cleanly even
          when the screen is in dark mode. */}
      <div className="player-sponsor-qr-wrap">
        {qrSvg ? (
          <div
            className="player-sponsor-qr"
            // The qrcode library returns trusted SVG for our own input —
            // no user-supplied content goes into it.
            dangerouslySetInnerHTML={{ __html: qrSvg }}
          />
        ) : (
          <div className="player-sponsor-qr-loading">Generating QR…</div>
        )}
      </div>

      <p className="player-sponsor-instruct">
        Family or friends nearby can <strong>scan this with their phone&apos;s
        camera</strong> to land on your sponsor page. Or share the link
        directly:
      </p>

      <div className="player-sponsor-actions">
        {shareSupported && (
          <button
            type="button"
            className="player-sponsor-btn-primary"
            onClick={handleShare}
          >
            📲 Share my page
          </button>
        ...
        <button
          type="button"
          className="player-sponsor-btn-secondary"
        ...
        <button
          type="button"
          className="player-sponsor-btn-tertiary"
          onClick={handleCopy}
        >
      ...
      <div className="player-sponsor-url-display">
        {url}
      </div>

      <p className="player-sponsor-finehint">
