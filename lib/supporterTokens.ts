// ============================================================
// Supporter token helpers (client-side).
//
...
// Mirrors the SQL function in 4.6-supporter-tokens.sql.
// Coaches see this so they know what supporters will see.
...
// Build the URL a supporter visits when they scan the QR.
// Uses the page origin so both production and preview deploys work.
export function supporterUrlFromToken(token: string): string {
  if (typeof window === "undefined") {
    // Server-side fallback (used only if a Server Component calls this)
    return `/supporter/${token}`;
  }
  return `${window.location.origin}/supporter/${token}`;
}
...
export async function ensureTokensForEvent(
  eventId: string
): Promise<{ tokens: SupporterTokenRow[]; players: PlayerForToken[] }> {
  ...
  const existingByPlayer = new Map<string, SupporterTokenRow>();
  ...
  const tokens = players
    ...
    .filter((t): t is SupporterTokenRow => Boolean(t));
